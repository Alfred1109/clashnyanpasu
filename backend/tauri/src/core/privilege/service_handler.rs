...
impl ServicePrivilegeHandler {
    ...
    async fn send_privileged_command(&self, operation: &PrivilegedOperation) -> Result<()> {
        ...
        match operation {
            PrivilegedOperation::SetSystemProxy { enable, port, bypass } => {
                self.set_system_proxy_via_service(*enable, *port, bypass.clone()).await
            }
            PrivilegedOperation::SetTunMode { enable } => {
                self.set_tun_mode_via_service(*enable).await
            }
            PrivilegedOperation::ResetSystemProxy => {
                self.reset_system_proxy_via_service().await
            }
            PrivilegedOperation::UpdateCorePermissions { core_path } => {
                self.update_core_permissions_via_service(core_path.clone()).await
            }
    ...
    async fn set_system_proxy_via_service(&self, enable: bool, port: u16, bypass: Vec<String>) -> Result<()> {
        ...
        let patch = crate::config::nyanpasu::IVerge {
            ...
            system_proxy_bypass: if bypass.is_empty() { None } else { Some(bypass.join(",")) },
            ..Default::default()
        };
    ...
    async fn update_core_permissions_via_service(&self, _core_path: std::path::PathBuf) -> Result<()> {
        info!("通过服务更新核心权限");
    ...
    fn supports_operation(&self, operation: &PrivilegedOperation) -> bool {
        match operation {
            ...
            PrivilegedOperation::ModifyNetworkSettings { .. } => {
                // 这些操作需要更多的服务端支持
                cfg!(feature = "advanced-service-operations")
            }
        }
    }
}
